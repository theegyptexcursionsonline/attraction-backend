...
Protocol 1 serializes namespace changes for each website in MongoDB transactions. Pages and tours reserve both their stored slug and tour pathSlug while they are on the website; drafts reserve their URLs, and archived or trashed records (status `archived`) release theirs so an editor can reuse the address on a new page or tour. Restoring a retired record claims its URL again through this protocol and is refused when another page or tour has taken it, so no two records ever serve one address. Retiring a record only releases a URL and therefore stays outside the protocol, remaining available while namespace writes are paused. The underlying page and attraction data remain authoritative. There is no claim backfill or destructive migration.
