...
export function safariLegacyPage(tenantId: string, slug: string, pages: any[], owners: any[]) {
  const mapping = tenantId === SAFARI_TENANT_ID ? SAFARI_LEGACY_PAGES[slug] : undefined;
  ...
  if (!page || page.status !== 'active' || page.isPublished !== true || !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(page.slug) || SAFARI_LEGACY_PAGES[page.slug]) return null;
